...
MainWindow::MainWindow(const QString& settingsFile)
    ...
    auto refreshPlan=[this]{auto* session=activeSession();if(!session)return;if(auto* viewport=qobject_cast<PlanViewport*>(documents_->currentWidget())){const render::SceneBuilder builder;viewport->setScene(builder.build(*session->projectStore().snapshot(),{{{-10000000,-10000000},{10000000,10000000}},session->activeFloorId()}));}updateSession();};
    auto report=[this,refreshPlan](runtime::CommitResult result,const QString& success){if(const auto* failure=std::get_if<runtime::CommitFailure>(&result)){statusBar()->showMessage(failure->diagnostics.empty()?"Command rejected":QString::fromStdString(failure->diagnostics.front().message));return false;}refreshPlan();statusBar()->showMessage(success);return true;};
    auto placeOpening=[this](architecture::OpeningKind kind){if(!activeSession())return;QDialog dialog(this);dialog.setWindowTitle(kind==architecture::OpeningKind::door?"Door tool":"Window tool");QFormLayout form(&dialog);QLineEdit width(kind==architecture::OpeningKind::door?"900":"1200");QComboBox pattern;if(kind==architecture::OpeningKind::door)pattern.addItems({"Single swing","Double swing","Sliding","Pocket","Open passage"});else pattern.addItems({"Fixed","Casement","Sliding"});form.addRow("Width (mm)",&width);form.addRow("Pattern",&pattern);QDialogButtonBox buttons(QDialogButtonBox::Ok|QDialogButtonBox::Cancel);form.addRow(&buttons);connect(&buttons,&QDialogButtonBox::accepted,&dialog,&QDialog::accept);connect(&buttons,&QDialogButtonBox::rejected,&dialog,&QDialog::reject);if(dialog.exec()!=QDialog::Accepted)return;const auto wide=parseNumeric(width.text().toStdString(),NumericField::length);if(!wide.canonicalValue||*wide.canonicalValue<=0){statusBar()->showMessage("Opening width must be a positive length");return;}architecture::OpeningSpec spec;spec.kind=kind;spec.width=geometry::Length(*wide.canonicalValue);if(kind==architecture::OpeningKind::door)spec.doorPattern=static_cast<architecture::DoorPattern>(pattern.currentIndex());else spec.windowPattern=static_cast<architecture::WindowPattern>(pattern.currentIndex());if(auto* viewport=qobject_cast<PlanViewport*>(documents_->currentWidget()))viewport->activateOpeningTool(spec);wallInput_->hide();statusBar()->showMessage(kind==architecture::OpeningKind::door?"Door · hover a wall for fit preview, then click":"Window · hover a wall for fit preview, then click");};
    edit->addAction(add("opening.door", "Place &Door…", QKeySequence("D"), "draw-rectangle", [placeOpening]{placeOpening(architecture::OpeningKind::door);}));
    edit->addAction(add("opening.window", "Place Wi&ndow…", QKeySequence("I"), "draw-rectangle", [placeOpening]{placeOpening(architecture::OpeningKind::window);}));
    edit->addAction(add("wall.split", "&Split selected wall…", {}, "edit-cut", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=1){statusBar()->showMessage("Select one wall to split");return;}QDialog dialog(this);dialog.setWindowTitle("Split wall");QFormLayout form(&dialog);QLineEdit offset("1000");form.addRow("Distance from start (mm)",&offset);QDialogButtonBox buttons(QDialogButtonBox::Ok|QDialogButtonBox::Cancel);form.addRow(&buttons);connect(&buttons,&QDialogButtonBox::accepted,&dialog,&QDialog::accept);connect(&buttons,&QDialogButtonBox::rejected,&dialog,&QDialog::reject);if(dialog.exec()!=QDialog::Accepted)return;auto parsed=parseNumeric(offset.text().toStdString(),NumericField::length);if(!parsed.canonicalValue){statusBar()->showMessage("Split distance is invalid");return;}report(session->walls().splitWall(*session->selection().begin(),geometry::Length(*parsed.canonicalValue)),"Wall split; hosted references remapped");}));
    edit->addAction(add("wall.merge", "&Merge selected walls", {}, "object-merge", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=2){statusBar()->showMessage("Select exactly two compatible walls");return;}auto it=session->selection().begin();const auto first=*it++,second=*it;report(session->walls().mergeWalls(first,second),"Walls merged; first stable ID retained");}));
    edit->addAction(add("wall.reverse", "Reverse wall direction", {}, "object-flip-horizontal", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=1){statusBar()->showMessage("Select one wall");return;}report(session->walls().reverseWall(*session->selection().begin()),"Wall reversed with opening orientation preserved");}));
    edit->addAction(add("opening.flip_hinge", "Flip door hinge", {}, "object-flip-horizontal", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=1){statusBar()->showMessage("Select one opening");return;}report(session->walls().flipOpening(*session->selection().begin(),true,false),"Opening hinge flipped");}));
    edit->addAction(add("opening.flip_swing", "Flip door swing side", {}, "object-flip-vertical", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=1){statusBar()->showMessage("Select one opening");return;}report(session->walls().flipOpening(*session->selection().begin(),false,true),"Opening swing side flipped");}));
    edit->addAction(add("wall.delete", "Delete selected wall", QKeySequence::Delete, "edit-delete", [this,report]{auto* session=activeSession();if(!session||session->selection().size()!=1){statusBar()->showMessage("Select one wall");return;}const auto id=*session->selection().begin();std::size_t hosted=0;for(const auto& [entityId,entity]:session->projectStore().snapshot()->entities)if(const auto* opening=std::get_if<architecture::Opening>(&entity);opening&&opening->hostWallId==id)++hosted;if(hosted&&QMessageBox::warning(this,"Delete hosted openings",QString("This wall hosts %1 opening(s). Delete the wall and the listed hosted openings?").arg(hosted),QMessageBox::Yes|QMessageBox::Cancel,QMessageBox::Cancel)!=QMessageBox::Yes)return;if(report(session->walls().deleteWall(id,hosted>0),hosted?"Wall and hosted openings deleted":"Wall deleted"))session->select({});}));
