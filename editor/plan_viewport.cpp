...
void PlanViewport::pointerDown(const PointerEvent& event){if(!session_||event.button!=PointerButton::left)return;if(wallMode_)return;auto hits=selectionQuery_.hitTest(*session_->projectStore().snapshot(),camera_,toDipSize(QSizeF(canvas_->size())),event.dip,session_->selection());if(!hits.empty()&&session_->selection().contains(hits.front().id)&&selectionQuery_.movable(*session_->projectStore().snapshot(),hits.front().id)){movingId_=hits.front().id;moveStartWorld_=event.world;moveDelta_={};session_->tools().pointerDown(event);canvas_->update();return;}selecting_=true;selectionStartDip_=selectionCurrentDip_=event.dip;session_->tools().pointerDown(event);canvas_->update();}
void PlanViewport::pointerMove(const PointerEvent& event){if(session_)session_->tools().pointerMove(event);if(movingId_)moveDelta_=event.world-moveStartWorld_;if(selecting_)selectionCurrentDip_=event.dip;if(session_)snap_=snaps_.resolve(*session_->projectStore().snapshot(),camera_,toDipSize(QSizeF(canvas_->size())),event.dip,snapOptions_);if(wallMode_)wallPreview_=snap_?snap_->world:event.world;canvas_->update();}
void PlanViewport::pointerUp(const PointerEvent& event){if(!session_||event.button!=PointerButton::left)return;if(wallMode_){const auto point=snap_?snap_->world:event.world;const auto junction=snappedJunction();if(!wallAnchor_){wallAnchor_=architecture::WallEndpoint{point,junction};wallPreview_=point;canvas_->update();return;}if(!geometry::near(wallAnchor_->position,point,geometry::distance(wallAnchor_->position,point).mm)){auto result=session_->walls().addSegment(session_->activeFloorId(),session_->wallLayerId(),*wallAnchor_,{point,junction});if(std::holds_alternative<runtime::CommitSuccess>(result)){refreshScene();wallAnchor_=architecture::WallEndpoint{point,session_->walls().lastEndJunction()};}}canvas_->update();return;}session_->tools().pointerUp(event);if(movingId_){if(geometry::norm(moveDelta_)>geometry::epsilon(geometry::norm(moveDelta_))){const auto* entity=session_->projectStore().snapshot()->find(*movingId_);if(entity){auto result=std::holds_alternative<architecture::Wall>(*entity)?session_->walls().translateWall(*movingId_,moveDelta_):session_->walls().moveJunction(*movingId_,moveStartWorld_+moveDelta_);if(std::holds_alternative<runtime::CommitSuccess>(result))refreshScene();}}movingId_.reset();moveDelta_={};canvas_->update();return;}if(!selecting_)return;selectionCurrentDip_=event.dip;auto selected=session_->selection();const double movement=std::hypot(selectionCurrentDip_.x-selectionStartDip_.x,selectionCurrentDip_.y-selectionStartDip_.y);if(movement<4){auto hits=selectionQuery_.hitTest(*session_->projectStore().snapshot(),camera_,toDipSize(QSizeF(canvas_->size())),event.dip,selected);if(!event.alt)hitCycle_=0;auto hit=selectionQuery_.cycle(hits,event.alt?hitCycle_++:0);if(!event.shift)selected.clear();if(hit){if(event.shift&&selected.contains(hit->id))selected.erase(hit->id);else selected.insert(hit->id);}}else{auto found=selectionQuery_.marquee(*session_->projectStore().snapshot(),camera_,toDipSize(QSizeF(canvas_->size())),selectionStartDip_,selectionCurrentDip_);if(!event.shift)selected=std::move(found);else for(const auto& id:found){if(selected.contains(id))selected.erase(id);else selected.insert(id);}}session_->select(std::move(selected));selecting_=false;canvas_->update();}
...
void PlanViewport::cancelInteraction(){selecting_=false;movingId_.reset();moveDelta_={};spaceHeld_=false;snap_.reset();snaps_.clear();if(session_)session_->tools().cancel();if(canvas_)canvas_->update();}
void PlanViewport::draw(QPainter& painter,const QSizeF& dipSize)const{
    ...
    if(movingId_&&session_){const auto snapshot=session_->projectStore().snapshot();const auto* entity=snapshot->find(*movingId_);painter.setPen(QPen(QColor("#2563EB"),2,Qt::DashLine));if(entity)if(const auto* selectedWall=std::get_if<architecture::Wall>(entity)){const auto a=std::get<architecture::Junction>(*snapshot->find(selectedWall->startJunctionId)).position+moveDelta_,b=std::get<architecture::Junction>(*snapshot->find(selectedWall->endJunctionId)).position+moveDelta_;const auto da=camera_.worldToDip(a,viewport),db=camera_.worldToDip(b,viewport);painter.drawLine(QPointF(da.x,da.y),QPointF(db.x,db.y));}else if(const auto* junction=std::get_if<architecture::Junction>(entity))for(const auto& [id,value]:snapshot->entities)if(const auto* incidentWall=std::get_if<architecture::Wall>(&value);incidentWall&&(incidentWall->startJunctionId==junction->header.id||incidentWall->endJunctionId==junction->header.id)){auto a=std::get<architecture::Junction>(*snapshot->find(incidentWall->startJunctionId)).position,b=std::get<architecture::Junction>(*snapshot->find(incidentWall->endJunctionId)).position;if(incidentWall->startJunctionId==junction->header.id)a=a+moveDelta_;else b=b+moveDelta_;const auto da=camera_.worldToDip(a,viewport),db=camera_.worldToDip(b,viewport);painter.drawLine(QPointF(da.x,da.y),QPointF(db.x,db.y));}}
